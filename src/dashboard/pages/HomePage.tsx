...
export const HomePage = () => {
  ...
  const handleBookRide = async () => {
    if (pickupAddress && dropAddress) {
      navigate(BOOKING_PATH.replace(":pickupAddress", pickupAddress).replace(":dropAddress", dropAddress));
    }
  }
          ...
          <form className="w-full" onSubmit={(e) => e.preventDefault()}>
            <div className=" flex flex-col mt-12 gap-4 ">
              <div className=" relative ">
                <input placeholder="Enter Pickup Location" className="p-5 pl-12.5 text-xl border-1 border-gray-400 rounded w-full" value={pickupAddress} onChange={(e) => { setPickupAddress(e.target.value); handlePickupSearch(e.target.value) }} />
              ...
              <div className="relative">
                <input placeholder="Enter Drop Location" className="p-5 pl-12.5 text-xl border-1 border-gray-400 rounded w-full" value={dropAddress} onChange={(e) => { setDropAddress(e.target.value); handleDropSearch(e.target.value) }} />
                <TbCircleDotFilled className="absolute w-6 h-6 -mt-11.5 ml-4 text-black" />
                {dropAddress.length > 3 && <GeoSearchResults
                  resultRef={resultRef}
                  results={dropResults}
                  onSelect={async (result) => {
              ...
              <div className="mt-6">
                <button className="bg-black text-white w-full text-2xl font-semibold py-5 rounded-md cursor-pointer" onClick={handleBookRide}>Book Ride</button>
              </div>
            </div>
          </form>
        </div>
